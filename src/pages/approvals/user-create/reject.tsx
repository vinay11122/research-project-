...
export default function RejectUserPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      setMessage('No rejection token found.')
      setError(true)
      setLoading(false)
      return
    }
    ...
    async function reject() {
      try {
        ...
        const res = await fetch(`${apiBase}/auth/approvals/user-create/reject?token=${encodeURIComponent(token)}`)
        ...
        setMessage(err.message || 'Failed to reject user')
        setError(true)
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
      <div className="max-w-md w-full space-y-6 bg-white p-8 rounded-xl shadow-lg border border-slate-200">
        <h2 className="text-center text-2xl font-bold text-slate-900">User Rejection</h2>
